...
if st.session_state.info_submitted and not st.session_state.prediction_submitted:
    ...
    with st.form("question_form"):
        ...
        # Kolom untuk tombol submit 
        col1, _ = st.columns([1, 1])  # Kolom kedua dikosongkan agar layout tetap rapi
        with col1:
...
if st.session_state.show_results:
    ...
    key = (processing_pred, perception_pred, input_pred, understanding_pred)
    if key in style_mapping:
        kode, link = style_mapping[key] 
        st.write("**Klik link berikut untuk menuju ke Course Calculus sesuai dengan Gaya Belajar Anda:**")
        st.markdown(f"""  
        🔗 [Link Course]({link})  
        """, unsafe_allow_html=True)  
        st.write("**Selamat belajar!**")
    else:
        st.warning("Kombinasi gaya belajar tidak dikenali dalam sistem.")

    st.markdown("---")
    if st.button("Ulangi Pengisian"):
        reset_forms()
        st.rerun()